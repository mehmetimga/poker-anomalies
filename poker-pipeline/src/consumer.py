...
def create_consumer(topic='poker-actions', bootstrap_servers='localhost:9092', max_retries=10, retry_delay=2):
    """
...
def consume_and_detect(topic='poker-actions', bootstrap_servers='localhost:9092', log_file='logs/anomalies.log'):
    """
    ...
    try:
        for message in consumer:
            ...
            print(f"{status} Player {player_id}: {event['action']:6s} ${event.get('amount', 0):6.2f} | "
        ...
        if suspicious_pairs:
            ...
            for pair_info in suspicious_pairs:
                ...
                print(f"  Joint anomalies: {pair_info['joint_anomalies']}/{pair_info['total_hands']}")
                print()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Kafka consumer for poker anomaly detection')
    parser.add_argument(
        '--topic',
